...
| 원본에서 잘라낸 10조각      | `public/documents/settlement-10-{cedar,willow,reed,ash,maple,elm,pine,oak,birch,yew}.webp` | 3-5 복원 작업대                      |

앞의 세 공문은 1024×1536이며 WebP 품질 94로 저장했습니다. 쪽지 원본은 1536×1024이며 손실 없는 WebP로 저장했습니다. 2026-09-09 요청에 따라 같은 원본을 sharp로 세로 10줄로 다시 잘랐습니다. 경계는 `round(i × 1536 / 10)`으로 정해 폭 153~154픽셀, 높이 1024픽셀의 손실 없는 조각 열 장을 별도 파일로 저장했습니다. 글자를 다시 그리거나 조각마다 독립 이미지를 생성하지 않았습니다. 화면에서도 각 조각의 실제 폭에 비례해 표시하므로 글줄 높이가 어긋나지 않습니다.

초기 화면은 `elm, birch, cedar, oak, reed, yew, maple, willow, pine, ash` 순서로 섞습니다. 이미 복원한 6조각 저장은 새 정답 순서의 완료 상태로 열리며 재플레이가 가능합니다. 미완료 저장은 옛 조각 순서를 유지한 채 대응하는 새 조각 묶음으로 펼쳐 10조각으로 변환합니다. 크롭 경계가 달라 이전 배치를 픽셀 단위로 그대로 유지하는 변환은 아닙니다. 진행 버전·저장 키·읽음·수집·답안·메모는 유지하고, 저장 실패 시 이전 저장 원문을 보존합니다. 복원 전에는 완성본 이미지와 전문이 표시되지 않습니다. 열 조각을 정답 순서로 연결한 결과와 원본의 모든 RGB 픽셀이 일치합니다. 크롭 좌표·크기·원본 해시는 [EPISODE3_ASSETS.json](EPISODE3_ASSETS.json)에 보관하며, 이전 6조각의 좌표는 `previousCrops`에 남겼습니다. 이전 배포의 캐시를 위해 옛 이미지 파일도 유지합니다.

아래 생성 프롬프트는 9월 8일 원본 생성 당시의 문구입니다. 9월 9일에는 원본을 재생성하지 않고 크롭 수만 변경했습니다.
